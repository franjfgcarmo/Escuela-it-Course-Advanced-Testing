...
public class Date
{
    ...
    {
        if (!new EquivalenceClasses.ClosedInterval.ClosedInterval(1, MonthLimit).Includes(month))
        {
            throw new ArgumentException($"{nameof(month)} is not valid");
        }

        if (!new EquivalenceClasses.ClosedInterval.ClosedInterval(1, DayLimit[month - 1]).Includes(day))
        {
